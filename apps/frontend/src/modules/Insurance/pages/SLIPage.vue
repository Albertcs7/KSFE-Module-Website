...
const searchItems = computed<SearchBarItem[]>(() => {
  ...
  sliStore.users.forEach(user => {
    if (!uniqueEmps.has(user.empCode)) {
      uniqueEmps.set(user.empCode, user)
    }
  })
...
const displayedUsers = computed(() => {
  if (selectedEmpCode.value) {
    return sliStore.users.filter(u => u.empCode === selectedEmpCode.value)
  }
...
const selectedEmpName = computed(() => {
  ...
  const user = sliStore.users.find(u => u.empCode === selectedEmpCode.value)
...
const displayedRemittances = computed(() => {
  if (selectedEmpCode.value) {
    return sliStore.remittances.filter(r => r.empCode === selectedEmpCode.value)
  }
  return []
})
...
const exportToCSV = (policy: SLIUser) => {
  ...
  const policyRemittances = sliStore.remittances.filter(
    r => r.empCode === policy.empCode && r.sliPolicyNumber === policy.sliPolicyNumber
  )
  ...
  <section class="sli-page">
    ...
    <header class="sli-header">
      ...
      <div class="action-buttons">
        <button class="btn-action primary" @click="isAddUserOpen = true">
          <svg viewBox="0 0 24 24" width="18" height="18" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"></path><circle cx="12" cy="7" r="4"></circle><line x1="20" y1="8" x2="20" y2="14"></line><line x1="23" y1="11" x2="17" y2="11"></line></svg>
          ADD
        </button>
    ...
    -->
    <SLIAddUserModal :is-open="isAddUserOpen" @close="isAddUserOpen = false" />

...
import { createPolicy, getGISPolicies } from '@/services/api/insurance.api'
...
// --- BACKEND DATA STATE ---
// Using local refs instead of mock store for read operations
const gisUsers = ref<GISUser[]>([])
...
/**
 * Fetch GIS users from backend
 */
const fetchGISUsers = async () => {
  isLoadingUsers.value = true
  loadError.value = ''
  try {
    const response = await getGISPolicies()
    // Assuming backend returns array of policies
    const data = Array.isArray(response.data) ? response.data : response.data.data || []

    // Transform backend response to GISUser format if needed
    gisUsers.value = data.map((policy: any) => ({
      empCode: String(policy.employee_code || policy.empCode),
      empName: policy.employee_name || policy.empName,
      gisPolicyNumber: policy.policy_no || policy.gisPolicyNumber,
      premium: policy.premium || 0,
      dateOfMaturity: policy.maturity_date || policy.dateOfMaturity,
    }))
  } catch (error) {
    console.error('Failed to fetch GIS users:', error)
    loadError.value = 'Failed to load GIS policies'
...
// Map our local users into the format required by the SearchBar component
...
const displayedUsers = computed(() => {
  if (selectedEmpCode.value) {
    ...
    const userPolicies = gisUsers.value.filter(u => u.empCode.toUpperCase() === searchCode)
...
const selectedEmpName = computed(() => {
  ...
  const searchCode = selectedEmpCode.value.toUpperCase()
  const user = gisUsers.value.find(u => u.empCode.toUpperCase() === searchCode)
...
const hasGISPolicy = computed(() => {
  ...
  const searchCode = selectedEmpCode.value.toUpperCase()
  return gisUsers.value.some(u => u.empCode.toUpperCase() === searchCode)
})
...
/**
 * ✅ UPDATED: Now sends data to backend instead of only store
 */
const handleAddUser = async (user: InsurancePolicyForm) => {
  try {
    // 🔥 Transform frontend form → backend payload
    const payload = {
      employee_code: Number(user.empCode),
      employee_name: user.empName,
      policy_no: user.policyNumber,
      policy_type: "GIS" as const, // ✅ IMPORTANT: comes from page, not modal
    ...
    // ✅ Refresh data from backend (no mock store)
    await fetchGISUsers()
  ...
  reader.onload = (e) => {
    ...
    try {
      ...
      const existingPolicyNumbers = new Set(
        gisUsers.value.map(user => user.gisPolicyNumber.trim().toUpperCase())
      )
    ...
    <header class="gis-header">
      ...
      <div class="action-buttons">
        <label
          ...
          title="Upload Old Data via CSV or XLS"
        >
          <svg
            viewBox="0 0 24 24"
            width="18"
